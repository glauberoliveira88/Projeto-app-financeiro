...
*(Nota: Este FSD consolida integralmente todas as decisões técnicas e funcionais necessárias para a implementação, tornando desnecessária a consulta a documentos externos).*
...
* *(Explicação: Este diretório representa a pasta do projeto versionada no repositório. Em servidores locais como XAMPP, pode ficar em `htdocs/simple-finance/`; em hospedagens como PythonAnywhere, fica na estrutura de diretórios privada/pública da aplicação).*
...
* **Views:** Responsáveis pela interface de usuário reativa construída em React e templates HTML base, estruturadas para consumir os dados fornecidos pelo backend. Os componentes e arquivos de build do React são organizados e compilados diretamente para a pasta de ativos estáticos (`app/static/js/` e `app/static/css/`) para entrega direta pelo Flask. Localizados na pasta `app/views/`, `app/static/` e `app/templates/`.
...
* Arquivos estáticos (CSS, JS compilado do React, imagens) em `app/static/`.
...
│   ├── views/            # Templates HTML / Componentes React base
│   └── static/           # Assets (JS compilado do React, CSS, imagens)
├── config/
...
* *Objetivo:* Diagnóstico centralizado de falhas na aplicação Flask e rastreio estruturado de eventos de segurança.
* *Comportamento:* Registro de exceções, falhas e eventos críticos em banco de dados, com mecanismo de contingência em arquivo quando o banco estiver indisponível.
...
* `logs_seguranca`: `id` (PK, INT, Auto Increment), `usuario_id` (FK para `usuarios`, Nullable), `evento` (VARCHAR), `ip` (VARCHAR, Nullable), `detalhes` (TEXT, Nullable), `created_at` (DATETIME).
...
* **Informações gravadas:** Data/hora, ID do usuário (quando identificado), tipo de evento de segurança, IP de origem e detalhes da tentativa.
* **Persistência:** Gravados de forma estruturada na tabela `logs_seguranca` do banco de dados MySQL.
...
5. Criação da estrutura de migrations (Alembic / Flask-Migrate) e tabelas do banco de dados (usuários, contas, categorias, lançamentos, recorrentes, logs_erros, logs_seguranca).
...
14. Implementação do sistema de logs de erro com contingência em arquivo e log de segurança estruturado no banco.
...
* [ ] Log de erros e log de segurança operacionais com contingência em arquivo e persistência em banco.
